...
void ScrApplyPlayerAnimation(RPCParameters* rpcParams)
{
    ...
    if (pPlayerPool)
    {
        ...
        if (pPlayerPed) {
            ...
            if (szAnimName[0] != '\0') {
                std::strncpy(pPlayerPed->m_szLastAppliedAnim, szAnimName, sizeof(pPlayerPed->m_szLastAppliedAnim) - 1);
                pPlayerPed->m_szLastAppliedAnim[sizeof(pPlayerPed->m_szLastAppliedAnim) - 1] = '\0';
                pPlayerPed->m_bHasAppliedAnim = true;
            }
...
void ScrAttachObjectToPlayer(RPCParameters* rpcParams)
{
	...
	// The legacy attach system is unreliable. We will convert this call to use the modern,
	// client-side attachment system. This requires destroying the original object
	// and creating a new one managed by the CPlayerPed.
	int modelId = pObject->GetModelIndex();
	pNetGame->GetObjectPool()->Delete(ObjectID);

	CPlayerPed* pTargetPed = nullptr;
	if (pPlayerPool->GetLocalPlayerID() == PlayerID)
	{
		CLocalPlayer* pLocalPlayer = pPlayerPool->GetLocalPlayer();
		if (pLocalPlayer) {
			pTargetPed = pLocalPlayer->GetPlayerPed();
		}
	}
	else
	{
		...
		if (pRemotePlayer) {
			pTargetPed = pRemotePlayer->GetPlayerPed();
		}
	}

	if (pTargetPed)
	{
		// The legacy RPC does not provide a slot, bone, scale, or colors.
		// We will use reasonable defaults.
		const int LEGACY_ATTACH_SLOT = 9; // Use a dedicated slot for these legacy calls.

		ATTACHED_OBJECT_INFO attachInfo;
		memset(&attachInfo, 0, sizeof(ATTACHED_OBJECT_INFO));

		attachInfo.dwModelId = modelId;
		attachInfo.dwBoneId_MP = 1; // Default to BONE_SPINE
		attachInfo.vecOffset = CVector(offsetX, offsetY, offsetZ);
		attachInfo.vecRotation = CVector(rX, rY, rZ);
		attachInfo.vecScale = CVector(1.0f, 1.0f, 1.0f);
		// attachInfo.dwColor is already zeroed by memset

		pTargetPed->AttachObject(&attachInfo, LEGACY_ATTACH_SLOT);
	}
...
void ScrSetPlayerAttachedObject(RPCParameters* rpcParams)
{
    ...
    if (playerId == pPlayerPool->GetLocalPlayerID())
    {
        CLocalPlayer* pLocalPlayer = pPlayerPool->GetLocalPlayer();
        if (pLocalPlayer) {
            pPed = pLocalPlayer->GetPlayerPed();
        }
    }
    else
    {
        CRemotePlayer* pRemotePlayer = pPlayerPool->GetAt(playerId);
        if (pRemotePlayer) {
            pPed = pRemotePlayer->GetPlayerPed();
        }
    }
...
void ScrApplyActorAnimation(RPCParameters* rpcParams)
{
	...
	if (pActor) {
		if (szAnimName[0] != '\0') {
			std::strncpy(pActor->m_szLastAppliedAnim, szAnimName, sizeof(pActor->m_szLastAppliedAnim) - 1);
			pActor->m_szLastAppliedAnim[sizeof(pActor->m_szLastAppliedAnim) - 1] = '\0';
			pActor->m_bHasAppliedAnim = true;
		}
