...
VEHICLEID CVehiclePool::FindNearestToLocalPlayerPed()
{
    ...
    {
        if (m_bVehicleSlotState[x] && m_bIsActive[x] && m_pVehicles[x] && m_pVehicles[x]->m_pVehicle)
        {
...
void CVehiclePool::Process()
{
    ...
    CLocalPlayer* pLocalPlayer = pNetGame ? pNetGame->GetPlayerPool()->GetLocalPlayer() : nullptr;

    for (VEHICLEID VehicleID = 0; VehicleID < MAX_VEHICLES; VehicleID++)
    {
        if (m_bVehicleSlotState[VehicleID] && m_bIsActive[VehicleID])
        {
            // Проверяем валидность vehicle
            if (!m_pVehicles[VehicleID] || !m_pVehicles[VehicleID]->m_pVehicle)
            {
                // Если vehicle невалидный, помечаем как уничтоженный
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        // Управление двигателем и светом
                        if (pNetGame && pNetGame->m_pNetSet && pNetGame->m_pNetSet->bManualVehicleEngineAndLight)
                        {
                        ...
                        // Обновляем указатель, если он изменился
