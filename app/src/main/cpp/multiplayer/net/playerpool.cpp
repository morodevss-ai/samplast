...
bool CPlayerPool::IsValidPlayerSkin(int iSkin)
{
	// Базовые скины 0-311
	if (iSkin >= 0 && iSkin <= 311)
		return true;

	// Специальные скины (CJ и другие)
	if (iSkin >= 312 && iSkin <= 318)
		return true;

	// Скины из дополнений
	if (iSkin >= 319 && iSkin <= 323)
		return true;

	// Дополнительные скины до 400 (включительно)
	if (iSkin >= 324 && iSkin <= 400)
		return true;

	// Скины выше 400 (некоторые моды)
	if (iSkin > 400 && iSkin <= 500)
		return true;
