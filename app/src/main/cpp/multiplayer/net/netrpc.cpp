...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	case REJECT_REASON_BAD_NICKNAME:
        if (pUI) pUI->chat()->addInfoMessage("ОТКЛОНЕНИЕ ПОДКЛЮЧЕНИЯ: недопустимое имя пользователя");
		if (pUI) pUI->chat()->addInfoMessage("Пожалуйста, выберите другое имя пользователя длиной от 3 до 20 символов");
		if (pUI) pUI->chat()->addInfoMessage("Пожалуйста, используйте только буквы a-z, A-z и цифры от 0 до 9");
		if (pUI) pUI->chat()->addInfoMessage("Для выхода используйте /quit или нажмите ESC и выберите «Выйти из игры»");
...
void WorldPlayerAdd(RPCParameters *rpcParams)
{
	...
	if (pPlayerPool)
	{
		...
		if (pRemotePlayer)
		{
			// Проверяем валидность скина
			int iValidSkin = iSkin;
			if (!pPlayerPool->IsValidPlayerSkin(iSkin)) {
				// Если скин невалидный, используем скин CJ (0) как fallback
				FLog("Warning: Invalid skin ID %d for player %d, using CJ skin", iSkin, playerId);
				iValidSkin = 0;
			}

			if (pRemotePlayer->Spawn(byteTeam, iValidSkin, &vecPos, fRotation, dwColor, byteFightingStyle))
			{
				FLog("WorldPlayerAdd");
				...
				if (pPlayerPed)
				{
					...
					// Принудительно обновляем модель игрока
					pPlayerPed->m_pPed->SetModelIndex(iValidSkin);
				}
...
void WorldVehicleAdd(RPCParameters* rpcParams)
{
	...
	if (VehicleInfo.iVehicleType < 400 || VehicleInfo.iVehicleType > 612) return;
