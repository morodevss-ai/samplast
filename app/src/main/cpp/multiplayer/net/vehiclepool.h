...
public:
    ...
    {
        if (VehicleID < MAX_VEHICLES && m_pGTAVehicles[VehicleID]) {
...
public:
    CVehicleGTA* m_pGTAVehicless[MAX_VEHICLES];
...
private:
    ...
    bool m_bPendingCreate[MAX_VEHICLES];
    NEW_VEHICLE m_pendingVehicles[MAX_VEHICLES];

    PLAYERID		m_lastUndrivenId[MAX_VEHICLES];
    uint32_t		m_dwLastUndrivenProcessTick[MAX_VEHICLES];
};

inline int CVehiclePool::GetCount()
{
    int count = 0;
    for (int i = 0; i < MAX_VEHICLES; i++)
    {
