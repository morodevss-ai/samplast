...
#define PRINT_CRASH_STATES(context) \
    ...
    FLog("1: libmultiplayer.so + 0x%X", context->uc_mcontext.arm_pc - g_libmultiplayer); \
    FLog("2: libmultiplayer.so + 0x%X", context->uc_mcontext.arm_lr - g_libmultiplayer);
#else
#define PRINT_CRASH_STATES(context) \
    ...
    FLog("1: libmultiplayer.so + 0x%llx", context->uc_mcontext.pc - g_libmultiplayer); \
    FLog("2: libmultiplayer.so + 0x%llx", context->uc_mcontext.regs[30] - g_libmultiplayer);
#endif
    ...
    static _Unwind_Reason_Code TraceFunction(_Unwind_Context* context, void* arg) {
        ...
        if (dladdr(reinterpret_cast<void*>(pc), &info) && info.dli_sname != nullptr) {
            FLog("[adr: %p samp: %p gta: %p] %s\n",
                     ...
                     reinterpret_cast<void*>(pc - g_libmultiplayer),
