...
//bike fix
#include "game/Entity/Cbike.h"
#include "game/Entity/tBikeHandlingData.h"
...
namespace {
    std::atomic_bool gWeaponCycleRequested(false);
}

void RequestWeaponCycle()
{
    gWeaponCycleRequested.store(true, std::memory_order_release);
}

static bool g_javaTabVisible = false;

static uint32_t ToArgbFromRgba(uint32_t rgba)
{
    return (rgba >> 8) | 0xFF000000;
}

static void PopulateJavaTabList()
{
    ...
    pJavaWrapper->ClearTab();

    PLAYERID localId = pPlayerPool->GetLocalPlayerID();
    const char* localName = pPlayerPool->GetLocalPlayerName();
    if (localName && localName[0]) {
        int localLevel = pPlayerPool->GetLocalPlayerScore();
        int localPing = pPlayerPool->GetLocalPlayerPing();
        uint32_t localColor = 0xFFFFFFFF;
        if (pPlayerPool->GetLocalPlayer()) {
            localColor = pPlayerPool->GetLocalPlayer()->GetPlayerColorAsARGB();
        }
        pJavaWrapper->SetTab(localId, localName, localLevel, localPing, localColor);
    }

    for (PLAYERID playerId = 0; playerId < MAX_PLAYERS; ++playerId) {
        if (!pPlayerPool->GetSlotState(playerId) || pPlayerPool->IsPlayerNPC(playerId)) {
            continue;
        }
        ...
        int level = pPlayerPool->GetPlayerScore(playerId);
        int ping = pPlayerPool->GetPlayerPing(playerId);
        uint32_t color = ToArgbFromRgba(pPlayerPool->GetAt(playerId)->GetPlayerColor());

        pJavaWrapper->SetTab(playerId, playerName, level, ping, color);
    }

    pJavaWrapper->CommitTab();
}

static void FillJavaTabList()
{
    if (!pNetGame) {
        return;
    }
...
void ShowJavaTabList()
{
    if (g_javaTabVisible || !pJavaWrapper) {
        return;
    }

    FillJavaTabList();
    pJavaWrapper->ShowTab();
    g_javaTabVisible = true;
...
void HideJavaTabList()
{
    if (!g_javaTabVisible || !pJavaWrapper) {
        return;
    }

    pJavaWrapper->HideTab();
    g_javaTabVisible = false;
    if (pUI) {
        pUI->setVisible(true);
    }
    if (pGame) {
...
bool IsJavaTabListVisible()
{
    return g_javaTabVisible;
}
...
uint16_t CPad__GetPedWalkLeftRight_hook(uintptr_t thiz)
{
    ...
    else
    {
        ...
        if(LocalPlayerKeys.wKeyLR > 10 || LocalPlayerKeys.wKeyLR < -10)
...
uint16_t CPad__GetPedWalkUpDown_hook(uintptr_t thiz)
{
    ...
    else
    {
        ...
        if(LocalPlayerKeys.wKeyUD > 10 || LocalPlayerKeys.wKeyUD < -10)
...
uint16_t CPad__GetAccelerate_hook(uintptr_t thiz)
{
    ...
    else
    {
        ...
        if(wAccelerate > 10)
            ...
            LocalPlayerKeys.bKeys[ePadKeys::KEY_SPRINT] = 0;
        if (wAccelerate >= 0xFF)
        {
            if (pPlayerPed)
            {
...
uint16_t CPad__GetBrake_hook(uintptr_t thiz)
{
    ...
    else
    {
        ...
        if(wBrake > 10)
            ...
            LocalPlayerKeys.bKeys[ePadKeys::KEY_JUMP] = 0;
        if (wBrake >= 0xFF)
        {
            if (pPlayerPed)
            {
...
void AllVehicles__ProcessControl_hook(uintptr_t thiz)
{
       ...
       pVehicle->pDriver != GamePool_FindPlayerPed())
    {
        ...
        {
            pVehicle->physicalFlags.bDisableMoveForce = 0;
        }
        else
        {
            if(call_addr == 0x693978)
                pVehicle->physicalFlags.bDisableMoveForce = 1;
        }
        CWorld::PlayerInFocus = byteCurPlayer;
        // bike lean

        if(call_addr == 0x682BC8 || call_addr == 0x68A9AC)
        {
            CBike* pBike = (CBike*)pVehicle;

            fSavedBikeLean = pBike->m_RideAnimData.m_fLeanAngle;
            dwSavedBikeUnk = pBike->m_RideAnimData.m_fBarSteerAngle;
        ...
        if(call_addr == 0x682BC8 || call_addr == 0x68A9AC)
        {
            ...
            pBike->m_RideAnimData.m_fLeanAngle = fSavedBikeLean;
            pBike->m_RideAnimData.m_fBarSteerAngle = dwSavedBikeUnk;

            if(pBike->m_RideAnimData.m_fBarSteerAngle < pBike->m_RideAnimData.m_fLeanAngle)
                pBike->m_RideAnimData.m_fLeanAngle = pBike->m_RideAnimData.m_fLeanAngle - (pBike->m_RideAnimData.m_fLeanAngle - pBike->m_RideAnimData.m_fBarSteerAngle) * 0.5;
        }
        CWorld::PlayerInFocus = 0;
    }
    else
...
Java_com_goldrp_game_ui_hud_Hud_onWeaponChanged(JNIEnv *env, jobject thiz) {
