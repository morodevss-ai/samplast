...
bool bTextDrawTextureSlotState[MAX_TEXT_DRAWS];
uintptr_t TextDrawTexture[MAX_TEXT_DRAWS];
...
// 0.3.7
void DestroyTextDrawTexture(int index, bool keepSlot)
{
	uintptr_t pTexture;
	...
	if (index >= 0 && index < MAX_TEXT_DRAWS)
	{
		...
		bTextDrawTextureSlotState[index] = keepSlot;
	}
}
