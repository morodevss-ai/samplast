...
void MaterialTextGenerator::Render(const char* text, const ImVec2& size, int font_size, bool bold, const char* fontname,
								   const ImColor& font_color, const ImColor& background_color, int text_align)
{
	if (pUI) {
		ImGui::NewFrame();

		std::string utf8_text = Encoding::cp2utf(text);
		const char* info = utf8_text.c_str();

		// TODO: use fontname and bold
		...
		{
			case 0: // LEFT
				text_pos = ImVec2(0.0f, y_pos);
				break;
			case 1: // CENTER
				text_pos = ImVec2((size.x - text_size.x) / 2, y_pos);
				break;
			case 2: // RIGHT
				text_pos = ImVec2(size.x - text_size.x, y_pos);
				break;
		}
...
RwTexture* MaterialTextGenerator::Generate(const char* text, int size, int font_size, bool bold, const char* fontname, uint32_t font_color, uint32_t background_color, int alignment)
{
	int width, height;
	...
	if (raster && bufferTexture)
	{
		...
		RwRGBA color;
		color.alpha = (background_color >> 24) & 0xFF;
		color.red = (background_color >> 16) & 0xFF;
		color.green = (background_color >> 8) & 0xFF;
		color.blue = background_color & 0xFF;

        RwCameraClear(m_camera, &color, 3);
		RwCameraBeginUpdate((RwCamera*)m_camera);
		DefinedState();
		Render(text, ImVec2(width, height), font_size, bold, fontname, fix_color(font_color), fix_color(background_color), alignment);
