...
public:
    ...
    static bool Init2(const char *pDatFile);
    ...
    // Добавленные методы для безопасности
    static bool IsValidPlayerSlot(uint8_t slot);
    static int FindFirstFreePlayerPedSlotSafe();

    void SetGameInputEnabled(bool b);
...
private:
    ...
    // Статические члены для многопоточности
    static inline std::queue<std::function<void()>> tasks;
    static inline std::mutex mtx;
};

// Константы
constexpr uint32_t MAX_VEHICLE_MODELS = 212;