...
#include "Animation/AnimBlendUtils.h"
#include "Streaming.h"
#include "World.h"
...
CPlayerPed::CPlayerPed()
{
    ...
    iSpecialAction = 0;
    m_bHasAppliedAnim = false;
    m_szLastAppliedAnim[0] = '\0';
...
void CPlayerPed::SetModelIndex(uint uiModel)
{
    ...
    // No need to change if model is the same
    if(m_pPed->m_nModelIndex == uiModel) return;

    auto oldModelId = m_pPed->m_nModelIndex;

    if (!CStreaming::TryLoadModel(uiModel))
    {
        FLog("Failed to load model %d", uiModel);
        return;
    }

    // The game's own SetModelIndex should handle everything,
    // including updating the m_nModelIndex member.
    // Directly modifying m_pPed->m_nModelIndex before this call was causing the crash.
    m_pPed->SetModelIndex(uiModel);

    // Now, we can release our reference to the old model.
...
void CPlayerPed::GiveWeapon(int iWeaponId, int iAmmo)
{
    ...
    if(uiWeaponModel != -1)
    {
        if(!pGame->IsModelLoaded(uiWeaponModel))
        {
            ...
            while(!pGame->IsModelLoaded(uiWeaponModel)) usleep(5);
        }

        *pbyteCurrentPlayer = m_bytePlayerNumber;

        // CPed::GiveWeapon
        ((void (*)(CPedGTA*, int, int, bool))(g_libGTASA+(VER_x32 ? 0x49F588+1:0x59525C)))(m_pPed, iWeaponId, iAmmo, true);
...
bool CPlayerPed::HasAmmoForCurrentWeapon()
{
    if(IsValidGamePed(m_pPed))
    {
        ...
        if(!pSlot) return true;

        // Melee types always have ammo.
        if(pSlot->dwType <= WEAPON_CANE ||
           pSlot->dwType == WEAPON_PARACHUTE ) return true;

        if(!pSlot->dwAmmo) return false;
    }
    return true;
}
void CPlayerPed::SetAmmo(eWeaponType byteWeapon, uint32 wAmmo)
{
    if (m_pPed)
    {
        CWeapon* WeaponSlot = FindWeaponSlot(byteWeapon);
        if (WeaponSlot) {
            WeaponSlot->dwAmmo = wAmmo;
        }
    }
}
uint16_t CPlayerPed::GetKeys(uint16_t *lrAnalog, uint16_t *udAnalog, bool clear)
{
    ...
    *udAnalog = LocalPlayerKeys.wKeyUD;
    ...
    wRet <<= 1;
    // KEY_FIRE
    if(HasAmmoForCurrentWeapon() && LocalPlayerKeys.bKeys[ePadKeys::KEY_FIRE]) wRet |= 1;
    wRet <<= 1;
    // KEY_CROUCH
...
void CPlayerPed::ClearAnimation()
{
    if (!m_pPed || !GamePool_Ped_GetAt(m_dwGTAId) || !m_bHasAppliedAnim) {
        return;
    }
    ...
    if (m_pPed->m_pRwClump && m_szLastAppliedAnim[0] != '\0') {
        BlendOutClumpAnimationByName(m_pPed->m_pRwClump, m_szLastAppliedAnim, -2.0f);
    }
...
void CPlayerPed::SetInterior(uint8_t byteInteriorId, bool bRefresh)
{
    ...
    else
    {
        ...
        if (bRefresh)
        {
            ...
            CUtil::EnsureTextureDatabasesRegistered();
        }
    }
}
...
bool CPlayerPed::IsInJetpackMode()
{
    ...
    return ScriptCommand(&is_actor_using_jetpack, m_bytePlayerNumber);
}
...
void CPlayerPed::ProcessCuffAndCarry()
{
    ...
    RpHAnimHierarchy* hierarchy = GetAnimHierarchyFromSkinClump(m_pPed->m_pRwClump);
    if(!hierarchy || !hierarchy->currentAnim) return;
...
void CPlayerPed::AttachObject(ATTACHED_OBJECT_INFO* pInfo, int iSlot)
{
    ...
    if (!CStreaming::TryLoadModel(pInfo->dwModelId))
    {
        FLog("AttachObject: Failed to load model %d", pInfo->dwModelId);
        return;
    }
