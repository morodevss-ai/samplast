...
namespace {
    constexpr uintptr kInvalidGtaId = static_cast<uintptr>(-1);
}
...
CVehicle::CVehicle(int iType, float fX, float fY, float fZ, float fRotation, bool bPreloaded, bool bSiren)
{
    ...
    if ((iType != TRAIN_PASSENGER_LOCO) &&
        ...
        if (!CStreaming::TryLoadModel(iType)) {
            FLog("CVehicle::CVehicle model %d not loaded", iType);
            return;
        }
             ...
             (iType == TRAIN_TRAM))
    {
        ...
        if (!CStreaming::TryLoadModel(TRAIN_PASSENGER_LOCO)) {
            FLog("CVehicle::CVehicle train model %d not loaded", TRAIN_PASSENGER_LOCO);
            return;
        }

        if (!CStreaming::TryLoadModel(TRAIN_PASSENGER)) {
            FLog("CVehicle::CVehicle train model %d not loaded", TRAIN_PASSENGER);
            return;
        }

        if (!CStreaming::TryLoadModel(TRAIN_FREIGHT_LOCO)) {
            FLog("CVehicle::CVehicle train model %d not loaded", TRAIN_FREIGHT_LOCO);
            return;
        }

        if (!CStreaming::TryLoadModel(TRAIN_FREIGHT)) {
            FLog("CVehicle::CVehicle train model %d not loaded", TRAIN_FREIGHT);
            return;
        }
        if (!CStreaming::TryLoadModel(TRAIN_TRAM)) {
            FLog("CVehicle::CVehicle train model %d not loaded", TRAIN_TRAM);
            return;
        }
    ...
    if (m_pVehicle)
    {
        if (GamePool_Vehicle_GetAt(m_dwGTAId))
        {
            ...
            if (iModel == 441
                ...
                || iModel == 501
                || iModel == 564
                || iModel == 611 // Добавлена проверка для ID > 611
                || iModel == 612 // Добавлена проверка для ID > 611
                || iModel == 613) { // Добавлена проверка для ID > 611
                return true;
            }
        }
    }
...
void CVehicle::AddComponent(int iComponentID)
{
    ...
    {
        pUI->chat()->addDebugMessage("Component: TryLoadModel failed for %d", iComponentID);
        throw std::runtime_error("Model not loaded");
    }

    // Убрана проверка is_component_available, так как она может не работать для ID > 611
    // Вместо этого сразу пытаемся добавить компонент
    ...
    // Проверяем результат
    if (dwRet == 0) {
        pUI->chat()->addDebugMessage("Component: Failed to add component %d", iComponentID);
    }
}
// 0.3.7
...
float CVehicle::GetBikeLean()
{
    if (m_pVehicle && GamePool_Vehicle_GetAt(m_dwGTAId)) {
        ...
        CBike *pBike = (CBike *) m_pVehicle;

        return pBike->m_RideAnimData.m_fLeanAngle;;
    }
    return 0.0f;
}
// 0.3.7
...
{
    if (!m_pVehicle || m_dwGTAId == kInvalidGtaId || m_dwGTAId == 0) {
        return false;
    }

    const int gtaId = static_cast<int>(m_dwGTAId);
    if (static_cast<uintptr>(gtaId) != m_dwGTAId) {
        return false;
    }

    return GamePool_Vehicle_GetAt(gtaId) != nullptr;
}
// 0.3.7
...
bool CVehicle::IsATrailer()
{
    ...
    return (nModel == 435 ||
            ...
            nModel == 610 ||
            nModel == 611 ||
            // Добавлены дополнительные ID для моделей > 611
            nModel == 612 ||
            nModel == 613 ||
            nModel == 614 ||
            nModel == 615 ||
...
void CVehicle::ApplyTexture(const char* szTexture, const char* szNew)
{
    ...
    m_szReplacedTextures[bID].pTexture = (RwTexture*)CUtil::LoadTextureFromDB("samp", szNew);
...
{
    if (!m_pVehicle) return 9999.0f;

    CPlayerPed* pLocalPlayer = pGame->FindPlayerPed();
    if (!pLocalPlayer || !pLocalPlayer->m_pPed) return 9999.0f;

    CVector vecVehiclePos = m_pVehicle->m_matrix->m_pos;

    CVector vecPlayerPos = pLocalPlayer->m_pPed->m_matrix->m_pos;

    float dx = vecVehiclePos.x - vecPlayerPos.x;
    float dy = vecVehiclePos.y - vecPlayerPos.y;
    float dz = vecVehiclePos.z - vecPlayerPos.z;
