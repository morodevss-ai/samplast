...
// تابع برای ایجاد سایه برای تمام اشیاء بازی
void CShadows::StoreShadowForAllObjects(CEntityGTA* entity) {
    // اگر موجودیت یک شیء باشد، سایه برای آن ایجاد کن
    if (entity && entity->IsObject()) {
        // اطمینان از اینکه شیء می‌تواند سایه داشته باشد
        // استفاده از عملیات بیتی برای تغییر متغیر در union
        entity->m_nFlags &= ~(1 << 7);  // خاموش کردن بیت m_bDontCastShadowsOn
        ...
        // استفاده از تابع StoreShadowToBeRendered برای ایجاد سایه
        StoreShadowToBeRendered(
            SHADOW_DEFAULT, 
            gpShadowCarTex, 
            pos, 
            ...
            false, // رسم روی آب؟
            1.0f, // مقیاس
            nullptr, // سایه بلادرنگ
            false // رسم روی ساختمان‌ها؟
        );
    }
}
