...
{
	if (!g_bAudioStreamThreadWorked || !bassStream || g_bAudioStreamStop)
		return;

	if (pGame->IsGamePaused())
	{
		BASS_ChannelSetAttribute(bassStream, BASS_ATTRIB_VOL, 0.0f);
	}
	else
	{
		BASS_ChannelSetAttribute(bassStream, BASS_ATTRIB_VOL, 0.5f);
	}
}
