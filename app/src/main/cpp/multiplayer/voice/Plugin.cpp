...
void Plugin::MainLoop()
{
    ...
    int defaultKey = pSettings->Get().iVoiceChatKey;

    if (!pUI || !pUI->voicebutton()) return;

    if (MicroIcon::IsShowed()) {
        if (!Plugin::muteStatus) {
            if (pUI->voicebutton()->recording() && Plugin::recordStatus == 0) {
            ...
            if (!pUI->voicebutton()->recording() && Plugin::recordStatus == 1) {
                if (PluginConfig::GetMicroEnable()) {
...
void Plugin::ControlPacketHandler(const ControlPacket& controlPacket)
{
    switch(controlPacket.packet)
    {
        ...
        case SV::ControlPacketType::createLStreamAtVehicle:
        {
            ...
            const auto& streamPtr = Plugin::streamTable[stData.stream] = MakeStreamAtVehicle(stData.color, stData.name, stData.distance, stData.target);
