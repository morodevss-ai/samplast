...
GPS*      pGPS= nullptr;
extern bool g_hudEditorOpenedFromMenu;
...
uintptr_t g_libmultiplayer = 0x00;
...
extern "C" {
JNIEXPORT void JNICALL Java_com_goldrp_game_core_Samp_initializeSAMP(JNIEnv *pEnv, jobject thiz)
{
    pJavaWrapper = new CJavaWrapper(pEnv, thiz);

}
JNIEXPORT void JNICALL Java_com_goldrp_game_core_Samp_onInputEnd(JNIEnv *pEnv, jobject thiz, jbyteArray str)
{
    if(pUI)
    {
...
JNIEXPORT void JNICALL Java_com_goldrp_game_core_Samp_onEventBackPressed(JNIEnv *pEnv, jobject thiz)
{
    if(pSettings)
    {
...
JNIEXPORT void JNICALL Java_com_goldrp_game_core_Samp_onTabClosed(JNIEnv *pEnv, jobject thiz)
{
    HideJavaTabList();
}
JNIEXPORT void JNICALL Java_com_goldrp_game_ui_dialog_DialogManager_sendDialogResponse(JNIEnv* pEnv, jobject thiz, jint i3, jint i, jint i2, jbyteArray str)
{
...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
    ...
    g_libmultiplayer = CUtil::FindLib("libmultiplayer.so");
    if (g_libmultiplayer == 0x00) {
...
extern "C"
JNIEXPORT void JNICALL Java_com_goldrp_game_core_Samp_onChatInputRequested(JNIEnv *pEnv, jobject thiz)
{
    if (pUI && pUI->keyboard() && pUI->chat()) {
        pUI->keyboard()->show(pUI->chat());
    }
}
extern "C"
JNIEXPORT void JNICALL Java_com_goldrp_game_core_Samp_onWeaponChanged(JNIEnv *pEnv, jobject thiz)
{
    RequestWeaponCycle();
}
extern "C"
JNIEXPORT void JNICALL Java_com_goldrp_game_core_Samp_onKeyboardHidden(JNIEnv *pEnv, jobject thiz)
{
    if (pUI && pUI->keyboard()) {
        pUI->keyboard()->onJavaHidden();
    }
}

extern "C"
JNIEXPORT void JNICALL
Java_com_goldrp_game_ui_hud_Hud_clickCameraMode(JNIEnv *env, jobject thiz) {
    if(!pNetGame)return;
