...
#include "../../settings.h"
#include "../../util/CUtil.h"  // Add this include for CUtil
...
public:
    VoiceButton() : Button("TALK", UISettings::fontSize() / 2) {
        ...
        // Fallback to font atlas if textures failed to load (prevents null raster crashes/empty draw)
        if (!m_texture_micro_on || !m_texture_micro_on->raster) m_texture_micro_on = nullptr;
        if (!m_texture_micro_off || !m_texture_micro_off->raster) m_texture_micro_off = nullptr;
    ...
    virtual void draw(ImGuiRenderer* renderer) override
    {
        ...
        if (recording()) {
            raster = m_texture_micro_on ? m_texture_micro_on->raster : nullptr;
        } else {
            raster = m_texture_micro_off ? m_texture_micro_off->raster : nullptr;
        }
        if (raster) {
