...
/// @brief Firebase Crashlytics NDK SocialApi, for Android apps which use native code.
///
/// This SocialApi is optional: It enables adding custom metadata to your native Crashlytics crash
...
namespace firebase {
	namespace crashlytics {
		...
		/// @brief Initialise the Crashlytics NDK SocialApi, for Android apps using native code.
		///
		/// This allows finer grained control of when the native SocialApi is initialized. Calling this
		/// function is not not strictly necessary as the SocialApi will be initialized on the first call
    ...
    do {                                                                \
        if (((expression)) == NULL) {                                   \
			...
			inline __crashlytics_context_t* __crashlytics_init() {
				...
				__CRASHLYTICS_NULL_ON_NULL(lib = dlopen("libcrashlytics-handler.so", RTLD_LAZY | RTLD_LOCAL));
