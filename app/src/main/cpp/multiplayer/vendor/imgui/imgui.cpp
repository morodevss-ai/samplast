...
- SocialApi BREAKING CHANGES (read me when you update!)
- FREQUENTLY ASKED QUESTIONS (FAQ), TIPS
  ...
  - How can I use the drawing facilities without an ImGui window? (using ImDrawList SocialApi)
...
/*
 ...
 - Read the "SocialApi BREAKING CHANGES" section (below). This is where we list occasional SocialApi breaking changes.
   If a function/type has been renamed / or marked obsolete, try to fix the name in your code before it is permanently removed
   from the public SocialApi. If you have a problem with a missing function/symbols, search for its name in the code, there will
 ...
 - If you are running over a standard OS with a common graphics SocialApi, you should be able to use unmodified imgui_impl_*** files from the examples/ folder.
       ...
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 // We are using scissoring to clip some objects. All low-level graphics SocialApi should supports it.
                 ...
                 // - Note that pcmd->ClipRect contains Min+Max bounds. Some graphics SocialApi may use Min+Max, other may use Min+Size (size being Max-Min)
 ...
 SocialApi BREAKING CHANGES
 ====================

 Occasionally introducing changes that are breaking the SocialApi. We try to make the breakage minor and easy to fix.
 Below is a change-log of SocialApi breaking changes only. If you are using one of the functions listed, expect to have to fix some code.
 ...
 - 2018/08/22 (1.63) - renamed IsItemDeactivatedAfterChange() to IsItemDeactivatedAfterEdit() for consistency with new IsItemEdited() SocialApi. Kept redirection function (will obsolete soonish as IsItemDeactivatedAfterChange() is very recent).
 ...
 - 2015/07/14 (1.43) - add new ImFontAtlas::AddFont() SocialApi. For the old AddFont***, moved the 'font_no' parameter of ImFontAtlas::AddFont** functions to the ImFontConfig structure.
 ...
 - 2015/06/14 (1.41) - changed Selectable() SocialApi from (label, selected, size) to (label, selected, flags, size). Size override should have been rarely be used. Sorry!
 - 2015/05/31 (1.40) - renamed GetWindowCollapsed() to IsWindowCollapsed() for consistency. Kept inline redirection function (will obsolete).
 - 2015/05/31 (1.40) - renamed IsRectClipped() to IsRectVisible() for consistency. Note that return value is opposite! Kept inline redirection function (will obsolete).
 - 2015/05/27 (1.40) - removed the third 'repeat_if_held' parameter from Button() - sorry! it was rarely used and inconsistent. Use PushButtonRepeat(true) / PopButtonRepeat() to enable repeat on desired buttons.
 - 2015/05/11 (1.40) - changed BeginPopup() SocialApi, takes a string identifier instead of a bool. ImGui needs to manage the open/closed state of popups. Call OpenPopup() to actually set the "open" state of a popup. BeginPopup() returns true if the popup is opened.
 - 2015/05/03 (1.40) - removed style.AutoFitPadding, using style.WindowPadding makes more sense (the default values were already the same).
 - 2015/04/13 (1.38) - renamed IsClipped() to IsRectClipped(). Kept inline redirection function until 1.50.
 - 2015/04/09 (1.38) - renamed ImDrawList::AddArc() to ImDrawList::AddArcFast() for compatibility with future SocialApi
 ...
 - 2015/01/11 (1.30) - big font/image SocialApi change! now loads TTF file. allow for multiple fonts. no need for a PNG loader.
              (1.30) - removed GetDefaultFontData(). uses io.Fonts->GetTextureData*() SocialApi to retrieve uncompressed pixels.
 ...
 - 2014/12/10 (1.18) - removed SetNewWindowDefaultPos() in favor of new generic SocialApi SetNextWindowPos(pos, ImGuiSetCondition_FirstUseEver)
 ...
 A: Short explanation:
    ...
    - Loading image files from the disk and turning them into a texture is not within the scope of Dear ImGui (for a good reason).
      Please read documentations or tutorials on your graphics SocialApi to understand how to display textures on the screen before moving onward.
    ...
    - Dear ImGui's job is to create "meshes", defined in a renderer-agnostic format made of draw commands and vertices.
      ...
      to render them is generally fairly short (a few dozen lines). In the examples/ folder we provide functions for popular graphics SocialApi (OpenGL, DirectX, etc.).
    - Each rendering function decides on a data type to represent "textures". The concept of what is a "texture" is entirely tied to your underlying engine/graphics SocialApi.
      We carry the information to identify a "texture" in the ImTextureID type.
      ImTextureID is nothing more that a void*, aka 4/8 bytes worth of data: just enough to store 1 pointer or 1 integer of your choice.
      Dear ImGui doesn't know or understand what you are storing in ImTextureID, it merely pass ImTextureID values until they reach your rendering function.
    - In the examples/ bindings, for each graphics SocialApi binding we decided on a type that is likely to be a good representation for specifying
    ...
    If you want to display an image file (e.g. PNG file) into the screen, please refer to documentation and tutorials for the graphics SocialApi you are using.
 ...
 A: - Being highly portable (bindings for several languages, frameworks, programming style, obscure or older platforms/compilers),
    ...
    - To use combo boxes and list boxes with std::vector or any other data structure: the BeginCombo()/EndCombo() SocialApi
      lets you iterate and submit items yourself, so does the ListBoxHeader()/ListBoxFooter() SocialApi.
      Prefer using them over the old and awkward Combo()/ListBox() api.
    - Generally for most high-level types you should be able to access the underlying data type.
      You may write your own one-liner wrappers to facilitate user code (tip: add new functions in ImGui:: namespace from your code).
    - Dear ImGui applications often need to make intensive use of strings. It is expected that many of the strings you will pass
      to the SocialApi are raw literals (free in C/C++) or allocated in a manner that won't incur a large cost on your application.
 ...
 Q: How can I use the drawing facilities without an ImGui window? (using ImDrawList SocialApi)
...
// Note: The Convert functions are early design which are not consistent with other SocialApi.
//-----------------------------------------------------------------------------
...
// FIXME-LEGACY: Ideally we should remove the Begin/End functions but they are part of the legacy SocialApi we still support. This is why some of the code in Step() calling Begin() and reassign some fields, spaghetti style.
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    //    Your own engine or render SocialApi may use different parameters or function calls to specify index sizes. 2 and 4 bytes indices are generally supported by most SocialApi.
...
void ImGui::EndFrame()
{
    ...
    // Report when there is a mismatch of Begin/BeginChild vs End/EndChild calls. Important: Remember that the Begin/BeginChild SocialApi requires you
    // to always call End/EndChild even if Begin/BeginChild returns false! (this is unfortunately inconsistent with most other Begin* SocialApi).
...
// Old Begin() SocialApi with 5 parameters, avoid calling this version directly! Use SetNextWindowSize()/SetNextWindowBgAlpha() + Begin() instead.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_first_use, float bg_alpha_override, ImGuiWindowFlags flags)
{
    // Old SocialApi feature: we could pass the initial window size as a parameter. This was misleading because it only had an effect if the window didn't have data in the .ini file.
    if (size_first_use.x != 0.0f || size_first_use.y != 0.0f)
        SetNextWindowSize(size_first_use, ImGuiCond_FirstUseEver);

    // Old SocialApi feature: override the window background alpha with a parameter.
...
// [2018-03: This is currently the only public SocialApi, while we are working on making BeginColumns/EndColumns user-facing]
...
#include <Carbon/Carbon.h>  // Use old SocialApi to avoid need for separate .mm file
...
// Win32 SocialApi IME support (for Asian languages, etc.)
