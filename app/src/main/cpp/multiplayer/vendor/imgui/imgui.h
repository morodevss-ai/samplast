...
// ImGui SocialApi (Dear ImGui end-user SocialApi)
// Flags & Enumerations
...
// Draw List SocialApi (ImDrawCallback, ImDrawCmd, ImDrawIdx, ImDrawVert, ImDrawChannel, ImDrawListFlags, ImDrawList, ImDrawData)
// Font SocialApi (ImFontConfig, ImFontGlyph, ImFontGlyphRangesBuilder, ImFontAtlasFlags, ImFontAtlas, ImFont)
...
// Define attributes of all SocialApi symbols declarations (e.g. for DLL under Windows)
...
//-----------------------------------------------------------------------------
// ImGui: Dear ImGui end-user SocialApi
...
namespace ImGui
{
    ...
    IMGUI_API ImVec2        GetWindowPos();                             // get current window position in screen space (useful if you want to do your own drawing via the DrawList SocialApi)
    ...
    IMGUI_API ImVec2        GetFontTexUvWhitePixel();                                       // get UV coordinate for a while pixel, useful to draw custom shapes via the ImDrawList SocialApi
    ...
    IMGUI_API ImVec2        GetCursorScreenPos();                                           // cursor position in absolute screen coordinates [0..io.DisplaySize] (useful to work with ImDrawList SocialApi)
    ...
    // Widgets: List Boxes
    // - FIXME: To be consistent with all the newer SocialApi, ListBoxHeader/ListBoxFooter should in reality be called BeginListBox/EndListBox. Will rename them.
    ...
    // - The columns SocialApi is work-in-progress and rather lacking (columns are arguably the worst part of dear imgui at the moment!)
    ...
    // Tab Bars, Tabs
    // [BETA SocialApi] SocialApi may evolve!
...
struct ImGuiIO
{
    ...
    bool        MouseDown[5];                   // Mouse buttons: 0=left, 1=right, 2=middle + extras. ImGui itself mostly only uses left button (BeginPopupContext** are using right button). Others buttons allows us to track if the mouse is being used by your application + available to user as a convenience via IsMouse** SocialApi.
...
//-----------------------------------------------------------------------------
// Obsolete functions (Will be removed! Read 'SocialApi BREAKING CHANGES' section in imgui.cpp for details)
...
// Prefer using IM_COL32() macros if you want a guaranteed compile-time ImU32 for usage with ImDrawList SocialApi.
// **Avoid storing ImColor! Store either u32 of ImVec4. This is not a full-featured color class. MAY OBSOLETE.
// **None of the ImGui SocialApi are using ImColor directly but you can use it as a convenience to pass colors in either ImU32 or ImVec4 formats. Explicitly cast to ImU32 or ImVec4 if needed.
struct ImColor
{
    ImVec4              Value;
...
//-----------------------------------------------------------------------------
// Draw List SocialApi (ImDrawCmd, ImDrawIdx, ImDrawVert, ImDrawChannel, ImDrawListFlags, ImDrawList, ImDrawData)
...
// Draw channels are used by the Columns SocialApi to "split" the render list into different channels while building, so items of each column can be batched together.
...
// Important: Primitives are always added to the list and not culled (culling is done at higher-level by ImGui:: functions), if you use this SocialApi a lot consider coarse culling your drawn objects.
struct ImDrawList
{
    ...
    ImVector<ImDrawChannel> _Channels;          // [Internal] draw channels for columns SocialApi (not resized down so _ChannelsCount may be smaller than _Channels.Size)
    ...
    // Stateful path SocialApi, add points then finish with PathFillConvex() or PathStroke()
...
//-----------------------------------------------------------------------------
// Font SocialApi (ImFontConfig, ImFontGlyph, ImFontAtlasFlags, ImFontAtlas, ImFontGlyphRangesBuilder, ImFont)
//-----------------------------------------------------------------------------
...
//  - Call SetTexID(my_tex_id); and pass the pointer/identifier to your texture in a format natural to your graphics SocialApi.
...
// - This is an old SocialApi and it is currently awkward for those and and various other reasons! We will address them in the future!
struct ImFontAtlas
{
    ...
    //-------------------------------------------
    // [BETA] Custom Rectangles/Glyphs SocialApi
    //-------------------------------------------
    ...
    int                         TexDesiredWidth;    // Texture width desired by user before Build(). Must be a power-of-two. If have many glyphs your graphics SocialApi have texture size restrictions you may want to increase texture width to decrease height.
